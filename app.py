...
# Firebase / Firestore
try:
    from firebase_config import db  # type: ignore[import-not-found]
except ImportError:  # pragma: no cover - optional local/dev setup
    db = None
...
def require_db():
    if db is None:
        return {
            "status": "error",
            "message": "Firebase is not configured. Please set up firebase_config.py."
        }
    return None
...
def register_user(data: RegisterRequest):
    ...
    user_ref = db_client.collection("users").document()
